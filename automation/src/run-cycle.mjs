...
async function main() {
  ...
  const hasMercadoLivre = Boolean(process.env.MERCADO_LIVRE_ACCESS_TOKEN?.trim());

  // Exit before reading sources or calling ingestion/database services.
  // DATABASE_URL and affiliate tags alone do not enable external APIs.
  if (!hasAmazonBR && !hasAmazonUS && !hasMercadoLivre) {
    console.warn('[curate] Modo passivo: APIs externas não configuradas. Catálogo existente preservado; nenhuma escrita executada.');
    process.exitCode = 0;
    return;
  }
